...
<label class="form-label mt-2">Descrição</label>
    ...
    <script>
                //pega o telefone pelo id e executa a funcao de cada teca digitada
        document.getElementById('telefone_contato').addEventListener('input', function (e) {
            let valor = e.target.value.replace(/\D/g, ''); //barra d para permitir so numero e g global remove tudo

            if (valor.length > 11) {
                valor = valor.slice(0, 11); //11 digitos pro lil bro 
            }
            ...
            if (valor.length <= 2) {
                ...
                valor = '(' + valor.slice(0, 2) + ') ' +
                        valor.slice(2, 7) + '-' +
                        valor.slice(7);
            }// a mascara do telefone com tamanho adequado

            e.target.value = valor;// mostra o formato no tempo certinho
        });
    </script>
</body>
</html>